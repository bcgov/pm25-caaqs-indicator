...
-   **Fine particulate matter levels met the Canadian Ambient Air
    Quality Standards at `r max(percent_achieved_annual, percent_achieved_24h)`
    ...
    `r n_stations_achieved_annual` of the `r n_stations_annual` stations for 
    which valid data was obtained, while the 24-hour standard was met at 
    `r n_stations_achieved_24h` of the `r n_stations_24h` stations (
    `r percent_achieved_24h`%) with sufficient data for analysis.
...
-   There was not enough data to estimate PM$_{\text{2.5}}$ levels for the
    Northeast air zone for this reporting period. mCurrently, there are no 
