...
---
title: "Status of Fine Particulate Matter in B.C. (`r params$year_range`)"
---
...
tfee_sites <- read_rds(here("data/datasets/print_tfee_sites.rds"))
plots <- read_rds(here("data/datasets/print_plots.rds"))
stn_plots <- read_rds(here("data/datasets/print_stn_plots.rds"))
station_summary <- read_rds(here("data/datasets/print_station_summary.rds"))
pm_data <- read_rds(here("data/datasets/pm25_results.rds"))
...
-   **Fine particulate matter levels met the Canadian Ambient Air
    Quality Standards at `r min(station_summary$percent_achieved)`
    ...
    `r station_summary$n_achieved[station_summary$metric == "pm2.5_annual"]` of the 
    `r station_summary$n[station_summary$metric == "pm2.5_annual"]` stations 
    (`r station_summary$percent_achieved[station_summary$metric == "pm2.5_annual"]`%) 
    for which valid data was obtained, while the 24-hour standard was met at 
    `r station_summary$n_achieved[station_summary$metric == "pm2.5_24h"]` of the 
    `r station_summary$n[station_summary$metric == "pm2.5_24h"]` stations 
    (`r station_summary$percent_achieved[station_summary$metric == "pm2.5_24h"]`%) 
...
-   Wildfires are generally the largest contributor to transboundary
    ...
    period, `r length(tfee_sites)` stations had one or more days during the wildfire season
...
```{r message=FALSE, results='asis', fig.height=3, fig.asp = 0.4}
...
for (az in unique(pm_data$airzone)) {
  sites <- unique(pm_data$site[pm_data$airzone == az])
  cat("\\subsection{", az, " Air Zone}", sep = "")
  
  for (s in sites) {
    
    caaqs_data_24h <- filter(pm_data, site == .env$s, metric == "pm2.5_24h")
    caaqs_data_annual <- filter(pm_data, site == .env$s, metric == "pm2.5_annual")
    
    ## Subset daily, caaqs, and annual data
    daily_plot <- stn_plots[[s]][["24h"]]
    annual_plot <- stn_plots[[s]][["annual"]]
    
    cat("\\subsubsection*{\\underline{", knitr_latex_char(s), " monitoring station}}", sep = "")
