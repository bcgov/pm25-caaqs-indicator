...
az_24h_colours <- c("Not Achieved (&gt; 28 &mu;g/m&sup3;)" = "#cd7277",
                    "Achieved (&leq; 28 &mu;g/m&sup3;)" = "#72a4cd",
                    "Insufficient Data" = "#dbdbdb")
...
# # Load and prep data
stations_annual <- read_sf(here("out/pm_caaqs_annual.geojson"))
stations_24h <- read_sf(here("out/pm_caaqs_24h.geojson"))

# Count stations before removing for insufficient data
az_n <- stations_annual %>%
  group_by(airzone) %>%
  count() %>%
...
stations_annual <- stations_annual %>%
  mutate(caaqs_ambient = replace(caaqs_ambient, is.na(caaqs_ambient), "Insufficient Data"),
         caaqs_legend = rcaaqs:::cut_management(metric_value_ambient, 
                                               "pm2.5_annual", "breaks_h")) %>%
  filter(caaqs_ambient != "Insufficient Data") %>% 
  mutate(caaqs_legend = factor(caaqs_legend,
                               levels = c("&lteq; 4&mu;g/m&sup3;", 
                                          "&gt; 4&mu;g/m&sup3; &amp; &lteq; 6.4&mu;g/m&sup3;", 
                                          "&gt; 6.4&mu;g/m&sup3; &amp; &lteq; 10&mu;g/m&sup3;", 
                                          "&gt; 10&mu;g/m&sup3;"),
...
stations_24h <- stations_24h %>%
  ...
  filter(caaqs_ambient != "Insufficient Data") %>%
  mutate(caaqs_legend = factor(caaqs_legend,
                               levels = c("&lteq; 10&mu;g/m&sup3;", 
                                         "&gt; 10&mu;g/m&sup3; &amp; &lteq; 19&mu;g/m&sup3;", 
                                         "&gt; 19&mu;g/m&sup3; &amp; &lteq; 28&mu;g/m&sup3;", 
                                         "&gt; 28&mu;g/m&sup3;"),
                               labels = names(station_24h_colours)))

# Load airzones
az_annual <- st_read(here("./out/pm_airzone_annual.geojson"), stringsAsFactors = FALSE) %>%
  left_join(az_n, by = "airzone")# Join only data, not by spatial

az_24h <- st_read(here("./out/pm_airzone_24h.geojson"), stringsAsFactors = FALSE) %>%
  left_join(az_n, by = "airzone")# Join only data, not by spatial

# Separate into distinct metrics and assign Insufficient data
az_annual <- az_annual %>% 
  rename(rep_station_name = station_name_ambient,
         rep_station_id = rep_stn_id_ambient, 
         n_years = n_years_ambient) %>%
  mutate(rep_station_id = paste0(rep_station_id, "_annual"),
         caaqs_ambient = replace(caaqs_ambient, is.na(caaqs_ambient), "Insufficient Data"),
         caaqs_legend = factor(caaqs_ambient, 
                               levels = c("Not Achieved", "Achieved", 
                                          "Insufficient Data"),
                               labels = names(az_annual_colours)),
         metric_value_ambient = as.numeric(metric_value_ambient))
...
az_annual <- mutate(az_annual, 
  ...
  mutate(popup = popup_caaqs(., type = "region", 
                             metric_name = "PM<sub>2.5</sub> Metric (annual)",
                             units = "&mu;g/m&sup3;",
                             standard_name = "PM<sub>2.5</sub> Air Quality Standard (annual)"),
...
az_24h <- mutate(az_24h, 
                 ...
                 p_station_id = rep_station_id) %>%
  mutate(popup = popup_caaqs(., type = "region", 
                             metric_name = "PM<sub>2.5</sub> Metric (24h)",
                             units = "&mu;g/m&sup3;",
                             standard_name = "PM<sub>2.5</sub> Air Quality Standard (24h)"),
         popup = replace(popup, n == 0, NA))
